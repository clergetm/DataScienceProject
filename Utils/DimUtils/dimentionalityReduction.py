...
def dim_reduc_protocol(pickle_filepath, file_name, folder_type) -> Tuple[pd.DataFrame, list]:
	"""
	...
	:param str folder_type: the current type running
	:returns: the x_dataset and the y_data generated by the protocol
	:rtype new_dataset, y: (pd.DataFrame, list)
	"""
	####################################################################################################################
	#                                                 VARIABLES                                                        #
	####################################################################################################################
	...
	for f in [pickle_file, plot_file]:
	...
	plt.title(f"{folder_type}Set {file_name}\nFeature Importances - Sum = "
	          f"{str(np.sum(importance_scores[indices[0:attribute_number_to_select]]))}")
